...
> `docs/decisoes-arquiteturais.md` (`AD-01..AD-38`), `docs/funcionalidades.md`,
...
um backend de microsserviços .NET 10 implantado em **Kubernetes** (Kustomize).
...
| Indicador | Valor |
| --- | --- |
| Projetos de aplicação/infra | **8** (`src/`) |
| Projetos de teste | **2** (`tests/`) |
| Testes automatizados | **58 verdes** — 46 unitários + 12 de integração (Testcontainers) |
...
| Decisões arquiteturais registradas | **AD-01..AD-38** |
...
### Fase 1 — Orquestração com Aspire (posteriormente revertida)

- **Problema:** 5 processos + Postgres + RabbitMQ + Elasticsearch são caros de subir à mão em dev.
- **Feito:** `ConexaoSolidaria.AppHost` subia o grafo inteiro com `dotnet run`, injetava connection
  strings/descoberta de serviço e publicava telemetria no Aspire Dashboard; `ServiceDefaults`
  compartilhado padroniza OTel, health e resiliência HTTP.
- **Desfecho:** o AppHost foi **removido** ao final do projeto. Com o Kubernetes consolidado como
  ambiente de execução, manter um terceiro descritor do mesmo grafo (AppHost + Compose + Kustomize)
  custava mais do que entregava, e a remoção ainda eliminou a dívida do MessagePack. O
  `ServiceDefaults` permanece e segue sendo a base de OTel/health/resiliência. Ver `AD-37`.
- **Evidência:** `AD-01` (supersedida), `AD-02`, `AD-37`.
...
| **Service discovery** | Nomes lógicos resolvidos por DNS do Kubernetes (`Microsoft.Extensions.ServiceDiscovery` no Gateway); NetworkPolicy reforça o isolamento (`AD-04`). |
...
**58 testes verdes** — 46 unitários + 12 de integração. Executáveis com `dotnet test ConexaoSolidaria.slnx`.
...
Dois caminhos — um parcial, para trabalhar no backend, e um completo:

### 8.1 Docker Compose (backend + observabilidade)

`docker compose up --build` (após `Copy-Item .env.example .env` e preencher os segredos). Sobe infra,
as duas APIs, o Worker e Grafana/Prometheus/Zabbix em portas fixas. **Não inclui `gateway` nem `web`** —
para exercitar a UI ou o roteamento do Gateway, use o Kubernetes (8.2). O schema é criado por
**EF Migrations** no start — ao vir de uma stack antiga criada com `EnsureCreated`, recriar os
volumes (`down -v`).

### 8.2 Kubernetes (Kustomize) — **ambiente completo, deploy validado ao vivo**
...
- ~~**MessagePack transitivo do Aspire (advisory)**~~ — **RESOLVIDA.** A dívida saiu junto com a
  remoção do AppHost (`AD-37`): o MessagePack deixou de existir no grafo de dependências e
  `dotnet list package --vulnerable --include-transitive` reporta **zero** pacotes vulneráveis na
  solução inteira.
- **Sem execução local completa fora do cluster** — com o AppHost removido e o Compose sem `gateway`/
  `web`, exercitar a UI ponta a ponta exige build de imagem + deploy no Kubernetes. Ciclo de feedback
  mais lento; adicionar os dois serviços ao Compose é um TODO em aberto (`AD-37`).
...
`docs/decisoes-arquiteturais.md` (`AD-01..AD-38`); para o contrato HTTP, `docs/api-reference.md`;
